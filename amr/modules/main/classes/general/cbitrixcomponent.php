...
class CBitrixComponent
{
	...
	{
		// var_dump('CBitrixComponent __construct');die;
	...
	final private function __getClassForPath($componentPath)
	{
		...
		if (!isset(self::$__classes_map[$componentPath]))
		{
			...
			if (file_exists($fname) && is_file($fname))
			{
				...
				{
					// var_dump($afterClasses[$i]);
					// var_dump(get_class($this));die;

					// $classParent = get_class($this);
					// $objParent = new $classParent;
					// $objChild  = new $afterClasses[$i];
					// var_dump(is_subclass_of($objChild, $classParent));
					// var_dump(is_subclass_of('CBitrixMenuComponent', "CBitrixComponent"));
					// die('for');

					// var_dump(is_subclass_of($afterClasses[$i], get_class($this)));die;

					if (is_subclass_of($afterClasses[$i], get_class($this)))
		...
		// var_dump(self::$__classes_map);die;
	...
	final public function initComponent($componentName, $componentTemplate = false)
	{
		...
		echo "<span style='display:block;padding:5px;color:#fff;background-color:#000;font-size:12px;'>$componentName</span>";

		// die('CBitrixComponent initComponent');
		...
		// var_dump($path2Comp);die;	//amr/menu
		if ($path2Comp == '')
		{
		...
		// var_dump($this->classOfComponent);
		// die('next?');

		// var_dump($this->classOfComponent);die;
		...
		// var_dump($this->__currentCounter);die;
	...
	final protected function __includeComponent()
	{
		...
		// var_dump($_SERVER["DOCUMENT_ROOT"].$this->__path."/component.php");die;
	...
	final public function includeComponent($componentTemplate, $arParams, $parentComponent)
	{
		...
		// echo '__ ' . $componentTemplate;die;

		// var_dump($this->classOfComponent);
		// die('CBitrixComponent includeComponent');
